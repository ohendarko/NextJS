...
const OnboardingForm = ({ userEmail }) => {
  ...
  // Service / Licensing / Certification Progress
  boardSpecialization: false,
  maintainLicensure: false,
  hasCompletedOnboarding: false,
  hasPausedOnboarding: false,
  fpgeeFormSubmitted: false,
  fpgecApplicationSubmitted: false,
  fpgecCertficateIssued: false,
  eceApplicationCompleted: false,
  eceEvaluationFeeePaid: false,
  courseDescriptionsSubmitted: false,
  goodStandingLetterSubmitted: false,
  officialTranscriptsSent: false,
  passportPhotoSubmitted: false,
  passportScanSubmitted: false,
  ...
  pharmacyLicenseSubmitted: false,
  pharmacyLicenseObtained: false,
  internshipHoursCompleted: false,

  // NAPLEX
  naplexEligibilityConfirmed: false,
  naplexTestRegistered: false,
  naplexPreparationCompleted: false,
  naplexCompleted: false,

  // MPJE
  mpjeEligibilityConfirmed: false,
  mpjeTestRegistered: false,
  mpjePreparationCompleted: false,
  mpjeTestCompleted: false,

  // TOEFL
  toeflTestRegistered: false,
  toeflTestCompleted: false,
  toeflScoresSent: false,
  toeflDuration: '',
  toeflType: '',
  ...
  const saveForLater = async (e: React.FormEvent) => {
    e.preventDefault();
    
    
    // Making the API call to save the form data for later
    ...
    try {
      ...
      const res = await fetch(`/api/user/later`, {
        method: 'POST',
        headers: {
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mt-8">
              ...
              {formData?.selectedPackage ? (
                <div className="mt-2">
                  <p className="font-medium">{servicePackages.find(pkg => pkg.id === currentSelection)?.name}</p>
                  <p className="text-pharma-blue font-bold mt-1">
                    {servicePackages.find(pkg => pkg.id === currentSelection)?.price}
                  </p>
                </div>
              ) : (
